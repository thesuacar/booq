...
- Audiobook orchestration and metadata management (`services/audiobook_manager.py`, `src/pdf_processor.py`, `src/utils/preprocessing_utils.py`, `src/utils/captioning_utils.py`)
...
2. User uploads a PDF and configures language. [^2][^1]
...
- **Add Book page**:
    ...
    - Select language populated from the orchestrator endpoint.
...
- **Languages**: `LANGUAGES`, `LANGUAGECODES` mapping human names to TTS language codes. [^2]
...
- Endpoints:
    ...
    - `POST /audiobooks/create` – main pipeline entry:
        ...
        - Assembles `AudiobookSettings` from language, voice and speed.
...
Encapsulates the audiobook creation workflow around a PDF:
    - Uses `PdfProcessor` (from `pdf_processor.py`) to extract text and metadata per page.
...
- **`pdf_processor.py` / `pdf_utils.py`**:
Provide utilities for page‑wise text extraction and metadata access for PDFs using PyPDF‑like readers, with functions that are also used in the tests.[^4]
...
Typical responsibilities:
    ...
    - Given text, language, voice and speed, either return a cached file or invoke the model/3rd‑party TTS to synthesize new audio.
...
3. **Add a new audiobook**
    ...
    - Choose the audiobook language from the dropdown, and optionally adjust default playback speed and voice in **Settings** first.[^1][^3] [^2]
...
4. **Browse and manage the library**
    - The **Library** view lists all created audiobooks from `datalibrary.json`, showing title, language, date added, status and duration.[^1] [^2]
